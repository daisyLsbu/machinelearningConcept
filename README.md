...
- **Tutorial_03.ipynb** — Demonstrates reading a CSV file "by hand" using Python's built-in `csv` module rather than Pandas: opening the file, reading it with `csv.reader`, separating the header row from the data rows, and doing basic plotting on the result. A good foundational contrast to the Pandas-based loading shown in `MachineLearningSelf.ipynb`.
- **Tutorial_06.ipynb** — Loads the ADNI/TADPOLE Excel dataset (`ADNI_Tadpole_data_tutorial.xlsx`) with `pandas.read_excel`, sets `Amyloid status` as the prediction target, and drops `PTID` (patient ID) and `SUVR` (a variable directly derived from amyloid status, so dropped to avoid data leakage) from the features. Trains a plain `LogisticRegression` classifier and evaluates it with precision, recall, F1, accuracy, confusion matrix, and ROC/AUC metrics, plus K-Fold cross-validation.
- **PropertyInvestmentAnalysis_Final.ipynb** — The most substantial addition: a complete housing price analysis and prediction project. Covers data loading and initial inspection, missing-value handling separately for numerical columns (mean/mode/manual imputation, e.g. `MasVnrArea`) and categorical columns (e.g. `Alley`, basement-related fields), exploratory analysis and correlation heatmaps, feature selection (`SelectKBest`, mutual information regression), dimensionality reduction and clustering (PCA, t-SNE, K-Means, Agglomerative/hierarchical clustering with dendrograms), and regression modeling with several algorithms (Linear Regression, SGDRegressor, Random Forest Regressor, Decision Tree Regressor), evaluated with metrics like MSE and R². Visualizations mix Matplotlib/Seaborn with interactive Plotly charts.
- **ML-Tutorials7.ipynb** — A root-level copy of the Colab feature-selection tutorial, covering `SelectKBest` with chi² and mutual information, `f_classif`, and sequential feature selection, applied ahead of a Logistic Regression classifier. Content overlaps with `ColabNotebooks/ML-Tutorials7.ipynb`, but the two copies have since diverged slightly — worth reconciling which one is current.
## Repo Structure
```
